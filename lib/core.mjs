...
// Baseline climatologica: fissa (WMO 1961-1990), stessa del grafico. NON viene
// scaricata sul percorso caldo: la media per punto è precalcolata come asset
// (public/data/om-climatology-1961-1990.json, generata da scripts/gen_om_climatology.mjs).
export const CLIMATOLOGY = { start: "1961-01-01", end: "1990-12-31" };
 ...
 * chiamata (~600/min, 10.000/giorno). Sul percorso caldo serve solo `recent`
 * (306 punti, 1 anno): chunking da 100 con pausa tra le richieste.
 * chunkSize riducibile per range lunghi (usato da gen_om_climatology.mjs). */
export async function fetchGridMeans(grid, period, chunkSize = OM_CHUNK_SIZE) {
  const means = [];
  for (let i = 0; i < grid.length; i += chunkSize) {
    ...
    if (i + chunkSize < grid.length) await sleep(OM_CHUNK_DELAY_MS);
  }
  return means;
}

/* Payload completo della griglia (oggetto, non stringa: il caller serializza).
 * climatologyMeans = array delle medie 1961-1990 per punto (ordine di buildGrid),
 * caricato dal caller dall'asset precalcolato. Sul percorso caldo si scaricano
 * solo gli ultimi 12 mesi. La chiave `baseline` resta per compatibilità col
 * frontend: ora contiene la climatologia per punto. */
export async function buildGridPayload(climatologyMeans) {
  const grid = buildGrid();
  const periods = buildPeriods();
  const recent = await fetchGridMeans(grid, periods.recent);
  return { fetchedAt: new Date().toISOString(), periods, grid, recent, baseline: climatologyMeans };
}
...
export async function noaaStationData(token, lat, lon) {
  ...
  // giorno estremo del periodo: `better(a, b)` è true se il valore `a` va tenuto.
  // In caso di pari valore vince la data più recente (righe in ordine crescente).
  const extreme = (type, better) => {
    const rs = rows.filter((r) => r.datatype === type);
    if (!rs.length) return null;
    const r = rs.reduce((a, b) => (better(a.value, b.value) ? a : b));
    return { date: r.date.slice(0, 10), value: r.value };
  };
  return {
    status: 200,
    body: {
      ...
      warmestDay: extreme("TMAX", (a, b) => a > b),
      coldestDay: extreme("TMIN", (a, b) => a < b),
    },
  };
}

/* ---------------- NOAA GSOM: storico annuale della stazione ---------------- */

/* Storico lazy di 30 anni con il dataset mensile GSOM. A differenza di GHCND,
 * GSOM consente intervalli di 10 anni; bastano quindi tre richieste per TAVG
 * anziché decine di pagine giornaliere. */
export async function noaaStationHistory(token, lat, lon) {
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
    return { status: 400, body: { error: "Parametri lat/lon mancanti o non validi" } };
  }
  const h = { token };
  ...
  const requiredStart = new Date(now);
  requiredStart.setFullYear(requiredStart.getFullYear() - 30);
  const stationsRes = await fetchWithRetry(
    `https://www.ncei.noaa.gov/cdo-web/api/v2/stations?datasetid=GSOM&extent=${extent}&limit=100`,
    { headers: h }
  );
  const stations = ((await stationsRes.json()).results || []).filter(
    (s) => s.mindate && s.maxdate && s.mindate <= fmtDate(requiredStart) &&
      s.maxdate >= fmtDate(new Date(Date.now() - 3 * 365 * 86400000))
  );
  if (!stations.length) {
    return { status: 404, body: { error: "Nessuna stazione GSOM vicina con almeno 30 anni di dati recenti" } };
  }
  const dist = (s) => {
    const dLat = (s.latitude - lat) * 111;
    const dLon = (s.longitude - lon) * 111 * Math.cos((lat * Math.PI) / 180);
  ...
  const st = stations.reduce((a, b) => (dist(a) <= dist(b) ? a : b));
  const maxDate = new Date(st.maxdate);
  const lastStationYear = maxDate.getUTCFullYear() - (maxDate.getUTCMonth() === 11 ? 0 : 1);
  const endYear = Math.min(now.getUTCFullYear() - 1, lastStationYear);
  const startYear = endYear - 29;
  const rows = [];
  for (let year = startYear; year <= endYear; year += 10) {
    const chunkEnd = Math.min(year + 9, endYear);
    const url =
      `https://www.ncei.noaa.gov/cdo-web/api/v2/data?datasetid=GSOM&stationid=${st.id}` +
      `&datatypeid=TAVG&startdate=${year}-01-01&enddate=${chunkEnd}-12-31&units=metric&limit=1000`;
    const res = await fetchWithRetry(url, { headers: h });
    rows.push(...((await res.json()).results || []));
  }
  const monthly = new Map();
  for (const row of rows) {
    if (row.datatype !== "TAVG" || !Number.isFinite(row.value)) continue;
    monthly.set(row.date.slice(0, 7), row.value);
  }
  const annual = [];
  for (let year = startYear; year <= endYear; year++) {
    const values = [];
    for (let month = 1; month <= 12; month++) {
      const value = monthly.get(`${year}-${String(month).padStart(2, "0")}`);
      if (value !== undefined) values.push(value);
    }
    // Un anno con almeno 10 mesi è abbastanza rappresentativo, ma la
    // completezza resta visibile al frontend.
    if (values.length >= 10) {
      annual.push({
        year,
        tavg: values.reduce((sum, value) => sum + value, 0) / values.length,
        months: values.length,
      });
    }
  }
  if (!annual.length) {
    return { status: 404, body: { error: "La stazione GSOM selezionata non ha anni sufficientemente completi" } };
  }
  return {
    status: 200,
    body: {
